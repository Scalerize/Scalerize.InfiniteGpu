...
Write-Host "[1/5] Checking WiX Toolset v6.0 installation..." -ForegroundColor Yellow
...
Write-Host "[3/5] Publishing desktop application..." -ForegroundColor Yellow
$publishArgs = @(
    "publish",
    $desktopCsproj,
    "-c", $Configuration,
...
# Step 4: Harvest files using WiX heat command
Write-Host "[4/5] Harvesting files from published output..." -ForegroundColor Yellow
...
# Step 5: Build the MSI installer using WiX v6.0
Write-Host "[5/5] Building MSI installer with WiX v6.0..." -ForegroundColor Yellow
...
# WiX v6.0 unified build command - include both Package.wxs and HarvestedFiles.wxs
$wixArgs = @(
    "build",
    $productWxs,
    $harvestWxs,
